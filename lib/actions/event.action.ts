...
import User from "../database/models/user.nodel"
...
export async function getAllEvents({ query, limit = 6, page, category }: GetAllEventsParams) {
    try {
        ...
        const eventsQuery = Event.find(conditions)
            .sort({ createdAt: 'desc' }) //to sort events from date of creation them
            .skip(skipAmount)
            .limit(limit)

        const events = await populateEvent(eventsQuery)

        //to find number of pages(Documents) that create after fetch all events
...
export async function getRelatedEventsByCategory({
    ...
    try {
        ...
        const skipAmount = (Number(page) - 1) * limit
        //$and means that there was more than one condition 
